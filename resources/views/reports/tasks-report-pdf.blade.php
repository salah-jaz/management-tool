...
<head>
    ...
    <title>{{ get_label('tasks_report', 'Tasks Report') }}</title>
    <style>
        body {
    ...
    <header>
        <div class="report-header">
            ...
            <div class="report-info">
                <h2>{{ get_label('tasks_report', 'Tasks Report') }}</h2>
                <?php
        ...
        <div class="report-content">
            <table class="summary-table">
                <tr>
                    ...
                    <td class="summary-item">
                        <div class="summary-label">{{ get_label('due_tasks', 'Due Tasks') }}</div>
                        <div class="summary-value">
                            {{ $summary->due_tasks ?? 0 }} ({{ number_format($summary->due_tasks_percentage ?? 0, 2) }}%)
                        </div>
                    </td>
                    <td class="summary-item">
                        <div class="summary-label">{{ get_label('overdue_tasks', 'Overdue Tasks') }}</div>
                        <div class="summary-value">
                            {{ $summary->overdue_tasks ?? 0 }} ({{ number_format($summary->overdue_tasks_percentage ?? 0, 2) }}%)
                        </div>
                    </td>
                    <td class="summary-item">
                        <div class="summary-label">{{ get_label('average_task_completion_days', 'Avg. Task Completion Days') }}</div>
                        <div class="summary-value">{{ $summary->average_task_duration }}</div>
                    </td>
                    <td class="summary-item">
                        <div class="summary-label">{{ get_label('urgent_tasks', 'Urgent Tasks') }}</div>
            ...
            <div class="section">
                <h2 class="section-title">{{ get_label('tasks_details', 'Tasks Details') }}</h2>
                <table>
                    <thead>
                        <tr>
                            ...
                            <th rowspan="2">{{ get_label('project', 'Project') }}</th>
                    ...
                    <tbody>
                        @if (empty($tasks))
                        <tr>
                            <td colspan="18" class="text-center">{{get_label('no_data_available','No data available')}}</td>
                        </tr>
                        @else
                        @foreach ($tasks as $task)
                        <tr>
                            <td>{{ $task->id }}</td>
                            <td class="word-wrap">{{ strip_tags($task->title) }}</td>
                            <td class="word-wrap">{!! $task->description ?? '-' !!}</td>
                            <td class="word-wrap">{{ strip_tags($task->project) }}</td>
                            <td>{{ $task->start_date }}</td>
                            <td>{{ $task->due_date }}</td>
                            <td>{!! $task->status !!}</td>
                            <td>{!! $task->priority !!}</td>
                            <td>{{ $task->time->total_days }}</td>
                            <td>{{ $task->time->days_elapsed }}</td>
                            <td>{{ $task->time->days_remaining }}</td>
                            <td>{{ $task->time->overdue_days }}</td>
                            <td>{!! $task->users !!}</td>
                            <td>{{ $task->total_users }}</td>
                            <td>{!! $task->clients !!}</td>
                            <td>{{ $task->total_clients }}</td>
